...
#run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_OldReliability.pdf", reliability, "Max Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0,1), ylab="Reliability (Mean & STDEV)", num_cols = 4)
run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_Reliability.pdf", calculatePostPresentationReliabilityCached, "Max Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0,1), ylab="Reliability (Mean & STDEV)", num_cols = 4)
run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_Stability.pdf", calculateStabilityCached, "Max Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0,1), ylab="Stability (Mean & STDEV)", num_cols = 4)
#run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_WeakReliability.pdf", weakReliability, "Max Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0,1), ylab="Reliability (Mean & STDEV)", num_cols = 4)
run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_Latency.pdf", getOffSlots, "Max Node Count", legendTopLeftCorner, pdfWidth, pdfHeight, fiveTestsLatencyXYRatio, ylim=c(0, 150), ylab="Latency (Mean & STDEV)", num_cols = 4)
