...
max_node_count_loaded <- lapply(max_node_count_test_suites, function(test_suite_vector) lapply(test_suite_vector, loadResultsFromTestSuitePath))


run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount.pdf", reliability, "Max Node Count", c(0.735, 0.80), 13, 5.5, xyratio=2, ylim=c(0,1), ylab="Reliability (Mean & STDEV)")
run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_WeakReliability.pdf", weakReliability, "Max Node Count", c(0.735, 0.80), 13, 5.5, xyratio=2, ylim=c(0,1), ylab="Reliability (Mean & STDEV)")
run(max_node_count_loaded, max_node_count_labels, "MaxNodeCount_Latency.pdf", getOffSlots, "Max Node Count", c(0.05, 0.80), 13, 5.5, 0.012, ylim=c(0, 150), ylab="Latency (Mean & STDEV)")
