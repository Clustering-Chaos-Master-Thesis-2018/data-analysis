...
#run(min_node_count_loaded, min_node_count_labels, "MinNodeCount_OldReliability.pdf", reliability, "Min Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0, 1, 0.20), ylab=reliabilityText)
run(min_node_count_loaded, min_node_count_labels, "MinNodeCount_Reliability.pdf", calculatePostPresentationReliabilityCached, "Min Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0, 1, 0.20), ylab=reliabilityText)
run(min_node_count_loaded, min_node_count_labels, "MinNodeCount_Stability.pdf", calculateStabilityCached, "Min Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0, 1, 0.20), ylab=stabilityText)
#run(min_node_count_loaded, min_node_count_labels, "MinNodeCount_WeakReliability.pdf", weakReliability, "Min Node Count", legendBottomLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsXYRatio, ylim=c(0, 1, 0.20), ylab=reliabilityText)
run(min_node_count_loaded, min_node_count_labels, "MinNodeCount_Latency.pdf", getOffSlots, "Min Node Count", legendTopLeftCorner, pdfWidth, pdfHeight, xyratio=fiveTestsLatencyXYRatio, ylim=c(0, 150, 20), ylab=latencyText)
